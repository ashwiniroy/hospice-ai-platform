import os

from openai import OpenAI
from dotenv import load_dotenv

from app.ai.rag.retriever import HospiceRetriever


load_dotenv()


class ManualRagService:

    def __init__(self):
        self.client = OpenAI(
            api_key=os.getenv("OPENAI_API_KEY")
        )

        self.retriever = HospiceRetriever()
        self.model = "gpt-4.1-mini"

    def ask(
        self,
        question: str,
        state: str | None = None,
        ownership: str | None = None,
        top_k: int = 5
    ):
        results = self.retriever.retrieve(
            query=question,
            top_k=top_k,
            state=state,
            ownership=ownership
        )

        context_parts = []

        for result in results:
            metadata = result.get("metadata", {})
            text = metadata.get("text", "")

            if text:
                context_parts.append(text)

        context = "\n\n---\n\n".join(context_parts)

        if not context:
            return {
                "question": question,
                "answer": (
                    "The available hospice data does not contain enough "
                    "relevant information to answer this question."
                ),
                "sources": []
            }

        prompt = f"""
You are a hospice provider information assistant.

Answer the user's question using ONLY the hospice provider
information provided in the context below.

If the requested information is not present in the context,
say that the available data does not contain enough information.

Do not invent:
- quality ratings
- rankings
- clinical recommendations
- patient-specific medical advice

Do not treat vector similarity scores as provider quality scores.

Context:
{context}

User Question:
{question}
"""

        response = self.client.responses.create(
            model=self.model,
            input=prompt
        )

        sources = []

        for result in results:
            metadata = result.get("metadata", {})

            sources.append({
                "id": result.get("id"),
                "score": result.get("score"),
                "organization_name": metadata.get("organization_name"),
                "doing_business_as_name": metadata.get(
                    "doing_business_as_name"
                ),
                "city": metadata.get("city"),
                "state": metadata.get("state"),
                "npi": metadata.get("npi"),
                "ccn": metadata.get("ccn"),
                "proprietary_nonprofit": metadata.get(
                    "proprietary_nonprofit"
                )
            })

        return {
            "question": question,
            "answer": response.output_text,
            "sources": sources
        }